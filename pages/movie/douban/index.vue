    ...
    <div class="scroll">
      ...
      <Scroll class="better-scroll"
        ...
        <div class="scroll-wrapper">
          <nuxt-link tag="div" :to="{path:`/movie/douban/info/${item.id}`}" v-for="(item,index) in doubanList"
                     :key="index" class="movie-item">
            ...
            <p class="movie-item-p">导演：<span v-for="(d,i) in item.directors" :key="i"
                                             class="movie-item-span">{{d.name}}</span></p>
            <p class="movie-item-p">豆瓣评分：{{item.rating.average}}</p>
            <p class="movie-item-p">上映时间：{{item.mainland_pubdate}}</p>
            <p class="movie-item-p">类型：<span v-for="(g,i) in item.genres" :key="i" class="movie-item-span">{{g}}</span>
            </p>
          </nuxt-link>
        </div>
      </Scroll>
...
export default {
  async fetch({ store }) {
    if (store.state.movie.doubanList.length === 0) {
      const res = await axiosPlugin.axios.get('douban/list', {
        params: {
          city: '杭州',
