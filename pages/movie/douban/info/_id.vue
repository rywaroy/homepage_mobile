<template>
  <div class="app-content">
    <div class="scroll single-box">
      ...
      <div class="movie-infos">地区：<span class="movie-infos-text" v-for="(item,index) in info.countries" :key="index">{{item}}</span></div>
      <div class="movie-infos">时长：<span class="movie-infos-text" v-for="(item,index) in info.durations" :key="index">{{item}}</span></div>
      <div class="movie-infos">类型：<span class="movie-infos-text" v-for="(item,index) in info.genres" :key="index">{{item}}</span></div>
      <div class="movie-infos">语言：<span class="movie-infos-text" v-for="(item,index) in info.languages" :key="index">{{item}}</span></div>
...
export default {
  async asyncData({route, store}){
      ...
      let info = await axiosPlus.axios.get('douban/info',{params:{id:route.params.id}})
      return{
        info:info.data.data
      }
  },
